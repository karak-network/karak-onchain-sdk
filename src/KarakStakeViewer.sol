...
contract KarakStakeViewer is Initializable, OwnableUpgradeable, UUPSUpgradeable, IStakeViewer {
    /* STORAGE */
    ...
    function setOracle(address erc20, Oracle calldata oracle) external onlyOwner {
    ...
    function getStakeDistributionUSDForOperators(
        ...
        for (uint256 i = 0; i < operators.length; i++) {
            ...
            address[] memory vaults = core.fetchVaultsStakedInDSS(operators[i], IBaseDSS(dss));
            ...
            for (uint256 j = 0; j < vaults.length; j++) {
                ...
                uint256 assetBalance = IERC20Metadata(asset).balanceOf(vaults[j]);
